...
function handlePlayers(e, spreadsheet, forceCompute) {
  ...
  if (forceCompute) {
    ...
    numRows = 0;
    while (
      spreadsheet.getRange("B" + (numRows + 2).toString()).getValue() != ""
    )
      numRows++;
  }
  ...
  for (var row = startRow; row < startRow + numRows; row++) {
    ...
    var firstCol = "A",
      lastCol = "K"; // first and last columns to consider
    var totalCols = 11;
    var bwsPpVals = [0, 0, 0]; // BWS-adjusted pp values
    var bwsAdditive = 15825;
    var bwsMultiplier = 0.9937;
    ...
    for (var i = 0; i < maxPlayers; i++) {
      ...
      bwsPpVals[i] =
        (ppVals[i] - bwsAdditive) *
          exp(bwsMultiplier, data.badges.length * data.badges.length) +
        bwsAdditive;
    }

    if (badRow || requiredPpl <= 0) {
      spreadsheet
        .getRange(firstCol + r + ":" + lastCol + r)
    ...
    spreadsheet
      .getRange(lastModifiedCol + r)
      .setValue(Utilities.formatDate(new Date(), "UTC", "MM/dd HH:mm:ss"));
